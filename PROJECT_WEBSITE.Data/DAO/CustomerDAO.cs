...
namespace PROJECT_WEBSITE.Data.DAO
{
    public class CustomerDAO
    {
        ...
        public IEnumerable<MCustomer> GetCustomerPaging(SearchNews search, int page, int pageSize)
        {
            ...
            {
                lst = lst.Where(t => t.CreateDate.HasValue && t.CreateDate.Value.Date >= search.TuNgay.Value.Date);
            }
            if (search.DenNgay.HasValue)
            {
                lst = lst.Where(t => t.CreateDate.HasValue && t.CreateDate.Value.Date <= search.DenNgay.Value.Date);
            }
